...
bool           __resilience__::Interface::isServerInit = false;
/* 
 * ===  FUNCTION  ======================================================================
 *         Name:  init
...
__resilience__::Interface::init ( int sockFd , uint32_t port ){
	...
	//check port registered or not
	 ...
	 if ( returnVal ){
		 //do the ipc server start part
		 if ( !isServerInit ) startIpcServer();
...
#endif
	//check whether server has benn init or not
	if ( !ipcSerIns.isServerStart () ){
		if ( ipcThread.joinable() ){
			ipcThread.join();
		}
		ipcThread = std::thread ( &IpcServer::createServer ,ipcSerIns );
	}
...
void
__resilience__::Interface::stopIpcServer( void ){
#ifdef DEBUG
	std::cout<<PRINT<<"Stop ipc server"<<std::endl;
#endif
	if ( !ipcSerIns.closeServer() )
		std::cerr<<PRINT<<"Not able to close IPC server\n";
	else{
		std::cout<<PRINT<<"Ipc server closed"<<std::endl;
	}
#ifdef DEBUG
	std::cout<<PRINT<<"Join thread"<<std::endl;
#endif
	try{
		if ( ipcThread.joinable() ){
			ipcThread.detach();
