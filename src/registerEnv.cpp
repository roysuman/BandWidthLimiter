...
static pthread_mutex_t registerEnvLock    = PTHREAD_MUTEX_INITIALIZER;
globals::JsonData                         RegisterEnv::jsonConfig;
...
regEnv::RegisterEnv::doRegister( ){
	pthread_mutex_lock ( &registerEnvLock);
#ifdef DEBUG
	...
	if ( atoi(configFlag.c_str() ) ){ //check whether config file read set/not
		returnVal = jsonConfig.readFromConfigFile();
	}
	else returnVal = false;
	if ( returnVal ){
		for ( int loop = 0 ; loop < jsonConfig.portCount ; ++ loop ){
			__limitsContainerIterator__ it = 
				limitStore.find ( jsonConfig.portBindData[loop].port );
			if ( it == limitStore.end() ){
				try{
					__limits__ tempLimit;// = new __limits__;
					tempLimit.upLimit = jsonConfig.portBindData[loop].upLimit;
					tempLimit.downLimit = jsonConfig.portBindData[loop].downLimit;
					tempLimit.bindPort = jsonConfig.portBindData[loop].port;
					tempLimit.ipAddress =jsonConfig.portBindData[loop].ip;
					tempLimit.latency = jsonConfig.portBindData[loop].latency;
				...
				std::cout<<PRINT<<"Port [ "<<jsonConfig.portBindData[loop].port<<" ] already registered"<<std::endl;
			}
		}
	}else if ( !atoi(configFlag.c_str())){
		returnVal = true;
		try{
	...
	pthread_mutex_unlock ( &registerEnvLock);
...
regEnv::RegisterEnv::modifyEntry( __updateLimits__ tempLimit	){
#ifdef DEBUG
	std::cout<<PRINT<<" Update port metrices "<<std::endl;
#endif
	__limitsContainerIterator__    it;
	bool                           returnVal;
	returnVal = true;
	...
	try{
		...
		if ( it != limitStore.end() ){
#ifdef DEBUG
		std::cout<<PRINT<<"Port [ "<<tempLimit.bindPort<<" ] registered and rady for updation"
			<<"UpLimit [ "<<tempLimit.upLimit<<" ] DownLimit [ "<< tempLimit.downLimit
			<<" ] "<<std::endl;
#endif
			if ( tempLimit.upLimit >= 0 ){
				it->second.upLimit = ( tempLimit.upLimit * 1000 );
			}
			if ( tempLimit.downLimit >= 0 ){
				it->second.downLimit = ( tempLimit.downLimit * 1000 );
			}
		}
		else{
